        ...
        while (true) {
            ...
            for (size_t i = 0; i < candidates.size(); ++i) {
                if (!candidates[i].visited) {
                    ...
                    // Overhead represents estimated time for stopping, picking up, or turning (2.0s)
                    double overhead = 2.0; 

                    // --- BUDGET CHECK ---
                    // Logic: Is (Elapsed Time + Time to Victim + Time to Exit + Buffer) <= Total Budget?
                    double projectedTotalTime = currentTime + timeToVictim + timeToExit + overhead;

                    if (projectedTotalTime <= time_limit) {
                        
                        // --- SELECTION HEURISTIC ---
                        // Efficiency Score = Reward / Cost
                        // We add 1.0 to timeToVictim to prevent division by zero and dampen the weight of very close nodes
